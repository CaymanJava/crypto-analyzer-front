...
@Component({
  moduleId: module.id,
  templateUrl: './price-momentum-oscillator.component.html'
})
export class PriceMomentumOscillatorComponent extends BaseIndicatorComponent {
  ...
  initForms() {
    this.configForm = this.fb.group({
      ...
      'doubleSmoothingPeriod': ['', [Validators.required, Validators.min(1), NotDecimalValidator.valid]],
  ...
  onSubmit() {
    this.modal.close({
      configuration: {
        ...
        doubleSmoothingPeriod: this.configForm.get('doubleSmoothingPeriod').value,
  ...
  fillConfiguration() {
    if (this.update) {
      this.configForm.setValue({
        ...
        doubleSmoothingPeriod: this.configuration.doubleSmoothingPeriod,
